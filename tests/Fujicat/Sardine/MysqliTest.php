...
class MysqliTest extends \PHPUnit_Framework_TestCase
{
	static protected $models = array(
		'Model_Mysqli_User',
		'Model_Mysqli_MailSendHistory',
	);

	static public function setUpBeforeClass() {
		echo "\n
###########################################
# Begining Test Case: MysqliTest...
###########################################\n";
	...
	static public function tearDownAfterClass() {
		...
		Model_Mysqli::disconnect();
		echo "Closed database connection.";

		echo "
###########################################
# Finished Test Case: MysqliTest...
###########################################\n";
	}
	...
	public function test_insert( $user ) {
		$obj = Model_Mysqli_User::forge($user)->save();
		...
		$hist = Model_Mysqli_MailSendHistory::forge(array(
	...
	public function test_multi_column_index() {
		...
		$indexes = Model_Mysqli_MailSendHistory::find($sql);
	...
	public function test_method_find() {
		...
		$users = Model_Mysqli_User::find($sql, array(
			'birth'	=> '1970-01-01',
		));
	...
	public function test_method_find_one_by_xxx() {
		$user = Model_Mysqli_User::find_one_by_name('Jaqueline');
	...
	public function test_method_find_one_by() {
		// 通常取得
		$user = Model_Mysqli_User::find_one_by('name', 'Jaqueline');
		$this->assertEquals($user->name, 'Jaqueline');
		$this->assertEquals($user->password, md5('jack'));
		$this->assertEquals($user->email, 'jack1972@gmail.com');
		$this->assertEquals($user->sex, 2);
		...
		// 条件にマッチしない場合はnullが返る
		$user = Model_Mysqli_User::find_one_by('name', 'Takoyaki');
	...
	public function test_method_find_by_xxx() {
		// 存在する名前を指定
		$users = Model_Mysqli_User::find_by_name('Takashi');
		// 2件取得できる
		...
		// 存在しない名前を指定
		$users = Model_Mysqli_User::find_by_name('Takoyaki');
		// 0件になる
	...
	public function test_method_find_by() {
		$users = Model_Mysqli_User::find_by('name', 'Takashi');
		$this->assertEquals(count($users), 2);
		$this->assertEquals($users[0]->birth, '1979-06-19');
		$this->assertEquals($users[1]->birth, '1972-02-23');
	}
	...
	public function test_update() {
		...
		// 改めて取得
		$updated = Model_Mysqli_User::find_one_by_name('Jaqueline');
		// メールアドレスが更新されている
		...
		$hist = Model_Mysqli_MailSendHistory::find_one_by_from_addr($email);
		$hist->to_addr = "hogehoge";
		$hist->save();

		// 改めて取得
		$updated = Model_Mysqli_MailSendHistory::find_one_by_from_addr($email);
	...
	public function test_delete() {
		// 取得してから削除する
		$user = Model_Mysqli_User::find_one_by_name('Keiko');
		$result = $user->delete();
