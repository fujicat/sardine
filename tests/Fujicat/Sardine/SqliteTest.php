...
class SqliteTest extends \PHPUnit_Framework_TestCase
{
	...
	static public function setUpBeforeClass() {
		echo "\n
###########################################
# Begining Test Case: SqliteTest...
###########################################\n";

		Model_Sqlite::connect();
		echo "Create database file: ".Model_Sqlite::database_filename().".\n";
	...
	static public function tearDownAfterClass() {
		echo "\n";
		Model_Sqlite::disconnect();
		echo "Closed database connection.\n";

		$database_filename = Model_Sqlite::database_filename();
		unlink($database_filename);
		echo "delete database file: {$database_filename}.";

		echo "
###########################################
# Finished Test Case: SqliteTest...
###########################################\n";
	}
		...
		foreach ( $lines as $line ) {
			...
			foreach ( $keys as $index=>$key ) {
				...
				switch ( $key ) {
					...
					case 'birth':		$record[$key] = strtotime($value);		break;
	...
	public function test_insert( $user ) {
		$obj = Model_Sqlite_User::forge($user)->save();
	...
	public function test_method_find() {
		...
		$users = Model_Sqlite_User::find($sql, array(
			'birth'	=> strtotime('1970-01-01'),
		));
		...
		foreach ( $users as $user ) {
			...
			$this->assertEquals(is_int($user->sex), true);
			$this->assertEquals(is_int($user->birth), true);
	...
	public function test_method_find_one_by_xxx() {
		$user = Model_Sqlite_User::find_one_by_name('Jaqueline');
		...
		$this->assertEquals($user->birth, strtotime('1972-03-09'));
	}
	...
	public function test_method_find_one_by() {
		// 通常取得
		$user = Model_Sqlite_User::find_one_by('name', 'Jaqueline');
		$this->assertEquals($user->name, 'Jaqueline');
		$this->assertEquals($user->password, md5('jack'));
		$this->assertEquals($user->email, 'jack1972@gmail.com');
		$this->assertEquals($user->sex, 2);
		...
		// 条件にマッチしない場合はnullが返る
		$user = Model_Sqlite_User::find_one_by('name', 'Takoyaki');
	...
	public function test_method_find_by_xxx() {
		// 存在する名前を指定
		$users = Model_Sqlite_User::find_by_name('Takashi');
		// 2件取得できる
		$this->assertEquals(count($users), 2);
		// 一人目の誕生日は1979-06-19
		$this->assertEquals($users[0]->birth, strtotime('1979-06-19'));
		// 二人目の誕生日は1972-02-23
		$this->assertEquals($users[1]->birth, strtotime('1972-02-23'));

		// 存在しない名前を指定
		$users = Model_Sqlite_User::find_by_name('Takoyaki');
		// 0件になる
	...
	public function test_method_find_by() {
		$users = Model_Sqlite_User::find_by('name', 'Takashi');
		$this->assertEquals(count($users), 2);
		$this->assertEquals($users[0]->birth, strtotime('1979-06-19'));
		$this->assertEquals($users[1]->birth, strtotime('1972-02-23'));
	}
	...
	public function test_update() {
		...
		// 改めて取得
		$updated = Model_Sqlite_User::find_one_by_name('Jaqueline');
		// メールアドレスが更新されている
	...
	public function test_delete() {
		// 取得してから削除する
		$user = Model_Sqlite_User::find_one_by_name('Keiko');
		$result = $user->delete();
